---
name: find-agents
description: 根据文档类型和任务目标在 .cursor/agents/ 目录下发现并推荐可并行调用的代理。当需要为文档生成、审查、验证等任务分派专家代理时使用。
---
...
在当前项目的 `.cursor/agents/` 目录中发现可用代理，按任务类型推荐并行代理组合及每个代理的"必答问题"。
...
- **禁止编造**：只返回 `.cursor/agents/` 下真实存在的代理文件名（不含 `.md`）。
