...
**信模型。** 信任大语言模型的推理能力，作为核心认知引擎，充分发挥其逻辑推导与上下文理解优势。

**惜注意。** 珍惜上下文窗口与 token 预算，避免冗余信息，每次交互聚焦单一目标，减少无效往返。

**验现实。** 任何结论必须经过可验证的证据支撑，不依赖假设，确保输出与客观事实一致。

**守底线。** 严守安全与合规边界，不生成有害内容，不绕过权限控制，保护敏感数据。

## 执行准则

**思在前。** 动手前先规划，明确目标、边界与验收标准，避免盲目编码。

## 退化对策

当复杂规则失效时，遵循先可见、后规则的退化路径：确保行为可观察、可调试，再追求规则完备性。
