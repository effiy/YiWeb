...
>
> **⛔ 完成以上两步后才算本次 generate-document 调用结束。**
...
7. **每次调用必须落盘**：无论成功、缺参、阻断或自检失败，本 skill 每次被调用都必须在 `docs/` 下创建或更新至少一个 Markdown 文件；无法确定目标文档路径时，写入 `docs/99_agent-runs/<YYYYMMDD-HHMMSS>_generate-document.md`。
...
| 类型 | 模板 | 规范 | 单独生成路径 | 全文档路径（`docs/<功能名>/`） | 依赖文档 | 生成方式 |
|------|------|------|------------|--------------------------------------|---------|---------|
...
每次调用 `generate-document` 都必须完成一次文件写入：

| 场景 | 必须写入的文件 |
|------|---------------|
| 单文档生成成功 | `docs/<阶段目录>/<功能名>.md` |
| 全文档生成成功 | `docs/<功能名>/` 下对应编号文档 |
| P0 自检失败但已生成正文 | 目标文档，顶部标注未通过项和待补充原因 |
| 缺少文档类型、功能名或上游材料导致无法生成目标文档 | `docs/99_agent-runs/<YYYYMMDD-HHMMSS>_generate-document.md` |
...
1. **解析请求** → 识别 `{文档类型, 功能名, 参考文档}`，缺失必填参数时，先写入兜底运行记录，再向用户澄清。
