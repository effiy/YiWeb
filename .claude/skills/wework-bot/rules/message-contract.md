...
会话与复盘（长流程完成/阻断类推送**建议**补齐，数据须可核对）：

- `⏱️ 用时` / `🟢 开始时间`：与本次执行真实起止一致；未记时则写「未记录」而非猜测秒数。
- `🪙 会话用量`：Token 或用量摘要，须与 Cursor 用量面板、账单或导出一致；无法取得时写「未提供，请从 Cursor 用量核对」，**禁止**编造。
...
- 必须包含 `🤖 模型：...`、`🧰 工具：...`、`🕒 最后更新：YYYY-MM-DD HH:mm:ss` 三行
