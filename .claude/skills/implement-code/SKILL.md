---
name: implement-code
description: 承接 generate-document 输出的文档，按"文档驱动 → 测试先行 → 动态检查门禁 → 模块预检全项目 → 写项目代码 → 代码评审 → 冒烟测试 → 过程总结"八阶段安全实施功能代码，**无论成功还是中途停止，必须生成 `docs/00_rYr/<功能名>/实施总结.md`**。当用户说"实施代码"、"开始开发"、"写代码"或输入 `/implement-code ...` 时必须使用。
---
...
10. **必须生成实施总结**：**无论流程是否成功完成，`docs/00_rYr/<功能名>/实施总结.md` 必须被生成**——正常完成时在阶段 7 生成完整总结；任何阶段因阻断停止时，在停止前先生成停止时总结（记录已执行阶段、产物、阻断原因和建议操作）。禁止在未生成实施总结的情况下终止流程。
...
> ⚠️ **强制要求**：`docs/00_rYr/<功能名>/实施总结.md` **必须在本阶段生成**。禁止跳过或以"口头总结"代替文件写入。
