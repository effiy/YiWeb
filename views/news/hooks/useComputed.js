...
// 需要外部传入 getDateString 和 categoriesConfig
export const useComputed = (store, { getDateString, categoriesConfig }) => {
    const {
        newsData,
        searchQuery,
        selectedCategories,
                ...
                if (category && category.news && Array.isArray(category.news)) {
                    const filteredNews = category.news.filter(item => {
                        ...
                        if (item.categories && item.categories.length > 0) {
                            ...
                            const duplicateTags = new Set((categoriesConfig || []).map(cat => cat.title));
