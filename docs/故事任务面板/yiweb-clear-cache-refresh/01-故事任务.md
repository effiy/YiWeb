...
flowchart TD
    ...
    SS --> CS["await 清除 CacheStorage<br/>JS/CSS/HTML 等静态资源"]
    CS --> RELOAD["location.reload()<br/>CacheStorage 已清除，直接刷新"]
    CONFIRM -->|取消| NOOP["无操作"]
```
...
- CacheStorage 已清除的情况下，直接 `location.reload()` 即可从服务器获取最新资源
- 页面刷新不保留表单状态
...
clearCacheAndRefresh() [async]
  ...
  ├─ 3. await caches.keys() → Promise.all(caches.delete(name)) 逐个清除并等待完成
  └─ 4. location.reload() 硬刷新
```

**核心逻辑**（`src/views/aicr/hooks/clearCacheMethods.js`）:
- 纯前端实现，无后端交互
- 四层缓存清理：localStorage（保留 Token/模型）→ sessionStorage → CacheStorage → 缓存爆破刷新
- CacheStorage 清除为异步操作，使用 `await Promise.all()` 确保所有删除完成后再刷新，消除竞态
- CacheStorage 已在步骤 3 中清除，`location.reload()` 直接重新加载页面，浏览器从服务器获取最新资源
- 使用原生 ES Modules 导出 async 函数
...
| 6 | 确认后硬刷新 | 点击确定 + Network 面板检查 | 资源未被浏览器缓存命中（200 非 304/from disk cache） |
