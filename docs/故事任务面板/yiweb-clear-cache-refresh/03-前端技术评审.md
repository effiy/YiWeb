...
| 组件/函数 | 输入 | 输出 | 副作用 |
|-----------|------|------|--------|
| `clearCacheAndRefresh()` | 无 | Promise\<void\> | 清除 localStorage（保留 Token/Model）、sessionStorage、CacheStorage（JS/CSS/HTML 静态资源）、缓存爆破刷新 |
...
用户点击按钮
  ↓
clearCacheAndRefresh() [async] 调用
  ├─ confirm() → 用户确认
  ├─ 遍历 localStorage → 移除非保留键
  ├─ sessionStorage.clear()
  ├─ await caches.keys() → Promise.all(caches.delete(name))
  └─ location.reload() 硬刷新
```
...
| 新增文件 | 导入位置 | 依赖上游 |
|----------|---------|---------|
| `src/views/aicr/hooks/clearCacheMethods.js` | `src/views/aicr/components/aicrPage/index.js` | 无外部依赖（仅 Browser API：localStorage/sessionStorage/CacheStorage） |
