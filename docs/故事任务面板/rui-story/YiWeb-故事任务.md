...
| # | 操作 | 触发条件 | 操作步骤 | 预期结果 |
|---|------|---------|---------|---------|
| 1 | 打开故事面板 | 用户访问故事面板页面 | 页面加载 → 自动请求远端故事数据 → 渲染看板视图 | 看到所有故事按状态（未开始/文档进行中/文档完成/编码进行中/编码完成/已阻断）分列展示的看板 |
...
| FP2 | 故事状态判定 — 基于文档存在性判定故事进度状态 | 故事的文件名列表 + 阻断状态 | 六种状态之一：未开始/文档进行中/文档完成/编码进行中/编码完成/已阻断 | 无法判定时默认显示原始状态值 | P0 |
...
| R# | 描述 | 校验方式 | 证据级别 |
|----|------|---------|---------|
| R1 | 故事状态按文档基线完整性递进判定：无故事任务文档=未开始，缺任一基线文档=文档进行中，基线齐备缺实施报告=文档完成，缺测试报告=编码进行中，全部齐备=编码完成 | 对照 `src/views/story/hooks/store.js:32-52` 判定逻辑验证 | B |
...
| 故事状态 | enum | `not_started` / `docs_in_progress` / `docs_done` / `code_in_progress` / `code_done` / `blocked` | store.js determineStatus() |
