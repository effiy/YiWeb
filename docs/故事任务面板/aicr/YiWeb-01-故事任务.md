...
| 6 | 标签过滤 | 点击标签按钮 | 1. 选择一个或多个标签 2. 文件树仅显示对应标签下的文件 | 过滤结果正确，支持反向过滤和无标签过滤 |
...
| 4 | 文件搜索与标签过滤 | FP6, FP7 | 实时过滤、反向过滤、无标签过滤 |
...
| AC7 | 用户选择反向过滤 | 切换过滤模式 | 文件树显示除选中标签外的所有文件 | Gate B |
...
|-----------|---------|------------|---------|------|
| §1 Story | 代码审查工作台整体需求 | 04-前端技术评审 | 组件架构 + 状态管理 + 交互设计 | 待生成 |
| §2 FP1-FP2 | 文件树与代码查看 | 04-前端技术评审 | 组件树 + 文件加载流程 | 待生成 |
| §2 FP3, FP5 | AI 对话与模型选择 | 04-前端技术评审 | 聊天模块 + 流式输出 | 待生成 |
| §2 FP4, FP12-FP14 | 会话管理与批量操作 | 04-前端技术评审 | 会话列表 + CRUD 方法 | 待生成 |
| §2 FP6-FP7 | 搜索与标签过滤 | 04-前端技术评审 | 搜索方法 + 标签过滤组件 | 待生成 |
| §5 AC1-AC12 | 全部验收标准 | 05-测试用例评审 | 测试用例全覆盖 | 待生成 |
