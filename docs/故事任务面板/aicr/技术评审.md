...
> | v3.0.0 | 2026-05-27 | deepseek-v4-pro | 🌿 feat/aicr | 📎 [CLAUDE.md](../../../CLAUDE.md) |
...
| # | 接口 | 方法 | 服务模块 | 用途 | 场景 |
|---|------|------|---------|------|:--:|
| 1 | `query_documents` | GET/POST | `data_service` | 查询会话/文档列表 | 1, 4, 5 |
| 2 | `create_document` | POST | `data_service` | 创建会话/文档记录 | 4, 5 |
| 3 | `update_document` | POST | `data_service` | 更新会话元数据/标签/消息 | 4 |
| 4 | `delete_document` | POST | `data_service` | 删除会话/文档记录 | 4, 5 |
| 5 | `chat` | POST (SSE) | `ai.chat_service` | AI 对话，流式 SSE 响应 | 2 |
| 6 | `list_ollama_models` | GET | `ai.chat_service` | 获取可用 AI 模型列表 | 2 |
| 7 | `/read-file` | POST | — | 读取文件内容（文本/图片） | 1, 5 |
| 8 | `/write-file` | POST | — | 创建或覆盖写入文件 | 5 |
| 9 | `/delete-file` | POST | — | 删除单个文件 | 5 |
| 10 | `/delete-folder` | POST | — | 递归删除文件夹 | 5 |
| 11 | `/rename-file` | POST | — | 重命名文件 | 5 |
| 12 | `/rename-folder` | POST | — | 重命名文件夹 | 5 |
...
| ID | Given | When | Then |
|----|-------|------|------|
| TC-S1-N01 | 后端 API 返回会话数据 | 页面加载 | 文件树从 session 数据构建并展示 |
| TC-S1-N02 | 文件树已加载，存在 .js 文件 | 点击 .js 文件 | 代码区展示语法高亮代码，行号可点击复制链接 |
| TC-S1-N03 | 文件树已加载，存在 .md 文件 | 点击 .md 文件 | 代码区渲染 Markdown，支持内部链接跳转 |
| TC-S1-N04 | 文件树已加载，存在 .png 文件 | 点击 .png 文件 | 弹出图片全屏预览弹窗 |
| TC-S1-N05 | URL 含 `?key=<path>` 参数 | 页面加载 | 自动展开目录层级，定位并高亮目标文件 |
| TC-S1-N06 | 文件树为树形视图 | 点击视图切换按钮 | 切换为卡片视图；再次点击切回树形 |
| TC-S1-N07 | 文件树已加载 | 输入搜索关键词 | 300ms 防抖后过滤结果，不匹配文件隐藏 |

**边界/异常:**

| ID | Given | When | Then |
|----|-------|------|------|
| TC-S1-E01 | 网络断开或 API 宕机 | 页面加载 | 错误状态 + 重试按钮，文件树显示空状态 |
| TC-S1-E02 | 文件树正常 | 点击文件后 API 返回 500 | 代码区显示错误占位，不影响文件树 |
| TC-S1-E03 | 文件树已加载 | 搜索不存在的文件名 | "未找到匹配文件" + 清除搜索按钮 |
...
| ID | Given | When | Then |
|----|-------|------|------|
| TC-S2-N01 | 已选中文件作为上下文，AI 服务可用 | 输入"解释这段代码"并发送 | SSE 流式响应逐块展示，文件内容作为上下文发送 |
| TC-S2-N02 | 上一条 AI 回复已展示 | 点击重新生成按钮 | 清除上一条回复，相同上下文重新请求 |
| TC-S2-N03 | AI 回复已完整展示 | 点击复制按钮 | 回复内容复制到剪贴板，显示复制成功提示 |
| TC-S2-N04 | 已选中文件 | 打开上下文编辑器，编辑内容，保存 | 下次聊天使用编辑后的上下文 |
| TC-S2-N05 | AI 服务可用 | 打开模型选择器 | 从 API 拉取可用模型列表，以下拉选项展示 |
| TC-S2-N06 | 模型列表为空或不可达 | 手动输入模型 ID 并确认 | 使用手动输入的模型 ID 发送请求 |
| TC-S2-N07 | 用户打开设置面板 | 编辑系统提示词内容，保存 | 下次聊天自动注入该提示词 |

**边界/异常:**

| ID | Given | When | Then |
|----|-------|------|------|
| TC-S2-E01 | AI 服务宕机 | 发送聊天消息 | 错误提示"模型服务不可用"，不展示空白回复 |
| TC-S2-E02 | 流式聊天进行中 | 网络断开 | "流式连接中断"提示，已接收部分保留 |
| TC-S2-E03 | 未选择任何文件 | 输入消息并发送 | 使用默认上下文正常发送，不报错 |
| TC-S2-E04 | AI 服务不可用 | 打开模型选择器或点击刷新 | "无法获取模型列表"提示，允许手动输入 |
| TC-S2-E05 | 用户清空系统提示词 | 保存空提示词并发送 | 请求中不注入系统提示词，正常发送 |
...
| ID | Given | When | Then |
|----|-------|------|------|
| TC-S3-N01 | 文件树包含多个项目目录 | 点击"YiWeb"项目标签 | 文件树仅显示 YiWeb 根目录，故事标签联动更新 |
| TC-S3-N02 | 已选择"YiWeb"项目标签 | 点击"aicr"故事标签 | 文件树缩小为 aicr 子目录，状态标签联动更新 |
| TC-S3-N03 | 已选择项目和故事标签 | 点击"技术评审"状态标签 | 仅显示匹配"技术评审"类型的文件 |
| TC-S3-N04 | 某项目根目录存在孤立文件 | 点击"无标签"筛选按钮 | 仅显示根目录下无子目录的文件 |
| TC-S3-N05 | 已应用任意筛选条件 | 按 Escape 键 | 所有筛选清除，文件树恢复完整列表 |
| TC-S3-N06 | 已应用任意筛选条件 | 点击清除筛选按钮 | 同 Escape，所有筛选条件重置 |

**边界/异常:**

| ID | Given | When | Then |
|----|-------|------|------|
| TC-S3-E01 | 已选择项目+故事+状态标签 | 组合下无任何匹配文件 | 空状态提示 + "清除筛选"快捷按钮 |
| TC-S3-E02 | 已选择"aicr"故事标签 | 快速切换选择"claude"故事标签 | 文件树立即更新，无闪烁或中间态 |
...
| ID | Given | When | Then |
|----|-------|------|------|
| TC-S4-N01 | 会话列表已加载 | 点击创建会话按钮 | 新会话出现在列表中，自动获得默认名称 |
| TC-S4-N02 | 会话列表中存在会话 | 编辑会话名称或描述，保存 | 会话元数据更新，列表刷新显示新名称 |
| TC-S4-N03 | 会话已存在 | 为会话添加标签"review"，再移除该标签 | 标签出现后消失 |
| TC-S4-N04 | 会话列表存在多个会话 | 进入批量模式，多选 3 个会话，点击删除并确认 | 3 个会话从列表移除，后端同步删除 |
| TC-S4-N05 | 会话列表中存在会话 | 点击会话的收藏按钮 | 标记为已收藏；再次点击取消收藏 |

**边界/异常:**

| ID | Given | When | Then |
|----|-------|------|------|
| TC-S4-E01 | 会话列表正常 | 删除会话时 API 返回 500 | "删除失败"提示，会话仍保留在列表中 |
| TC-S4-E02 | 已选中 3 个会话批量删除 | API 返回部分成功 | 成功的移除，失败的保留并提示具体失败项 |
...
| ID | Given | When | Then |
|----|-------|------|------|
| TC-S5-N01 | 右键点击文件夹打开上下文菜单 | 选择"新建文件"，输入文件名，确认 | 新文件出现在树中，后端同步创建 |
| TC-S5-N02 | 右键点击父目录 | 选择"新建文件夹"，输入名称，确认 | 新文件夹出现在树中 |
| TC-S5-N03 | 右键点击文件或文件夹 | 选择"重命名"，输入新名称，确认 | 树节点名称更新，后端同步重命名 |
| TC-S5-N04 | 右键点击文件或文件夹 | 选择"删除"，确认 | 节点从文件树中移除，后端同步删除 |
| TC-S5-N05 | 点击工具栏"导入文件夹" | 选择本地文件夹并确认 | 文件夹内容上传到后端，文件树显示新内容 |
| TC-S5-N06 | 文件树已加载 | 点击"导出文件夹"或"项目 ZIP 下载" | 浏览器下载 ZIP 包 |

**边界/异常:**

| ID | Given | When | Then |
|----|-------|------|------|
| TC-S5-E01 | 文件树中存在文件 A | 将文件 B 重命名为 A 的名称 | "文件名已存在"错误提示，文件 B 保持原名 |
| TC-S5-E02 | 选择了本地文件夹 | 上传过程中网络中断 | "导入失败"错误提示，文件树保持不变 |
| TC-S5-E03 | 上传项目 ZIP | ZIP 文件格式损坏 | "ZIP 文件无效"错误提示 |
