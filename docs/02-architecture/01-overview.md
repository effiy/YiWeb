...
在“无构建、纯 ESM + CDN Vue”前提下，本项目用目录与约定来建立边界。核心目标是：

- 高内聚：同一类变化集中在同一层/同一目录（页面装配、组件、网络、基础设施各自收敛）
- 低耦合：依赖方向单向；跨层通过“窄接口”交互；避免在低层直接引用高层的具体实现
- 副作用收敛：DOM 操作、滚动、剪贴板等 UI 副作用集中在 UI 层；数据层只产生“意图/请求”

建议的依赖方向（从上到下）：

- `views/*`（页面装配 + 业务编排）→ 只依赖 `components/*`、`services/*`、`utils/*`
- `components/*`（可复用 UI）→ 只依赖 `utils/view/*`（加载/注册）与少量 `utils/core/*`
- `services/*`（请求与业务服务）→ 只依赖 `config.js` 与 `utils/core/*`（错误/日志/基础工具）
- `utils/core/*`（基础设施）→ 尽量不依赖 `view/*`、不依赖具体页面/组件

全局 `window.*` 的定位：

- 作为“无构建环境”的兼容桥接层存在（例如组件注册、日志、部分 API）
- 业务逻辑内部尽量通过模块导入来使用能力，避免对全局变量产生硬依赖
...
当某个动作需要触发 UI 侧效应（例如滚动到底部/滚到某条消息），推荐做法是：

- 数据/业务方法只写入一个“请求对象”（例如写入 store 的某个 `ref`）
- UI 层监听该 `ref` 并执行真实的 DOM 操作

这样可以让：

- 业务逻辑保持可复用、可测试（不依赖具体 DOM 结构）
- DOM 选择器与滚动策略集中管理，避免在多处散落
...
本仓库当前已按“先低风险、后结构化”的策略实施了一轮优化（不改变运行方式、不引入构建）：

- 视图装配层减少对 `window.safeExecute` 这类全局符号的硬依赖，优先通过模块导入使用基础能力
- 组件等待机制优先使用组件注册事件进行同步，减少轮询带来的不确定性与耦合点
