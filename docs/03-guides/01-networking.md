...
- 分层目标：让页面侧尽量只关心“业务动作”，把协议兼容、错误处理、超时与鉴权收敛到 `services`
- 兼容目标：同时覆盖 JSON CRUD 与流式输出（SSE/分块输出），并能兼容服务端多版本 payload 结构
- 安全目标：token 仅放在请求头（`X-Token`），不写入 URL，减少泄露面；401 统一处理避免页面各自重复实现
- 可调试目标：保留请求/响应日志与耗时，debug 模式下可快速定位“卡住/超时/401/422 回退”等问题
...
- 缓存策略：目前 `getData` 做了简单内存缓存；可进一步按“资源类型/失效策略”拆分缓存（TTL、基于 ETag/Last-Modified）
- 错误语义：为常见错误（401/403/404/422/5xx/网络断开）定义统一的错误结构，页面按错误类型做一致提示与降级
- 流式健壮性：为流式解析补齐“断线重连/部分结果保留/心跳超时”策略，避免长对话中断体验差
- 观测与开关：把日志、耗时打点与重试策略都放在配置里（debug 开关、采样率、最大重试次数）
