...
// 模块依赖改为全局方式
// import { window.showGlobalLoading, window.hideGlobalLoading } from '/src/utils/loading.js';
// import { window.showError, window.showSuccess } from '/src/utils/message.js';
// import { window.getData, window.postData } from '/src/services/index.js';
                    ...
                    if (Array.isArray(response.data) && response.data.length > 0) {
                        await Promise.all(
                            response.data.map(rawItem => {
                                ...
                                return window.postData(`${window.API_URL}/`, payload);
                            })
                        );
            ...
            try {
                ...
                if (task && (task.id || task.key)) {
                    try {
                        ...
                        console.log('[任务更新] API更新 payload:', payload);
                        
                        // 调用API更新任务 - 使用统一的 postData 方法
...
`.trim();
                ...
                // 获取系统提示词
                const fromSystem = await window.getData(`/src/assets/prompts/target/taskContext.txt`);
                ...
                const saveResult = await window.postData(`${window.API_URL}/`, payload);
