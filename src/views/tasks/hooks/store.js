...
// 模块依赖改为全局方式
// import { getData, deleteData } from '/src/services/index.js';
...
window.createStore = () => {
    ...
    const loadTasksData = async () => {
        ...
        return window.safeExecuteAsync(async () => {
            ...
            try {
                ...
                if (featureName) queryParams.featureName = featureName;
                if (cardTitle) queryParams.cardTitle = cardTitle;
                
                // 使用 standardized service interface
                const queryUrl = window.buildServiceUrl('query_documents', queryParams);
    ...
    const deleteTask = async (task) => {
        return window.safeExecuteAsync(async () => {
            ...
            // 构建删除请求 payload
            const payload = {
                module_name: window.SERVICE_MODULE,
            ...
            console.log('[deleteTask] 删除 payload:', payload);
            
            // 调用API删除任务
            const deleteResult = await window.postData(`${window.API_URL}/`, payload);
            
            console.log('[deleteTask] API删除结果:', deleteResult);
            
            // 验证删除结果
            if (deleteResult && (deleteResult.code === 200 || deleteResult.success !== false)) {
                // 记录删除前的任务数量
