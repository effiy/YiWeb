...
const componentOptions = {
    ...
    css: [
        ...
        '/src/components/business/commentsList/index.css'
    ],
    html: '/src/components/business/commentsList/index.html',
        props: {
            // 可选：按新闻链接过滤（后续可由父级传入）
            newsLink: {
                type: String,
        ...
        methods: {
            ...
            openInAicr(comment) {
                try {
                    // 目标路径：/src/views/aicr/index.html
            ...
            async loadComments() {
                try {
                    ...
                    const params = {
                        cname: 'comments',
                        createdTime: `${encodeURIComponent(dateStr)},${encodeURIComponent(dateStr)}`
                    };
                    if (this.newsLink) {
...
(async function initComponent() {
    try {
        const CommentsList = await defineComponent(componentOptions);
