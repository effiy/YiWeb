...
const sanitizeMarkdownHtml = (html) => {
    try {
        ...
        const transformComponents = (rootEl) => {
            ...
            const handleSteps = () => {
                const stepsNodes = Array.from(rootEl.querySelectorAll('steps'));
                ...
                const stepNodes = Array.from(rootEl.querySelectorAll('step'));
...
export const renderMarkdownHtml = (text, options = {}) => {
    try {
        ...
        try {
            ...
            return sanitizeMarkdownHtml(window.marked(raw));
        } catch (_) {
            return escapeHtml(raw).replace(/\n/g, '<br/>');
        }
    } catch (_) {
